Title: About me
Date: 2015-07-12 19:59
Author: Elias Dorneles
Slug: about

Hello, I'm Elias Dorneles, a human originally from Brazil 🇧🇷️, living in France 🇫🇷️ since 2017.

## Music

Music is important to me: both consuming and making it. It feeds my soul.

You can listen to some of [my compositions here](/pages/music.html).

## Programming

I work as a software developer, notably with [Python](https://www.python.org)
and web technologies.

In 2017 I've attended the [Recurse Center](https://www.recurse.com), a retreat for programmers in NYC. There I learned a lot, made great friends and came out of it a braver programmer.

I currently work for [Back Market](https://www.backmarket.fr). The views expressed in this blog are mine and mine alone, and do not represent my
employer in any way.

You can find me on [GitHub](https://github.com/eliasdorneles) or [Stackoverflow](https://stackoverflow.com/users/149872/elias?tab=profile).

<a href="https://stackexchange.com/users/50263">
<img src="https://stackexchange.com/users/flair/50263.png?theme=clean" width="208" height="58" alt="profile for elias on Stack Exchange, a network of free, community-driven Q&amp;A sites" title="profile for elias on Stack Exchange, a network of free, community-driven Q&amp;A sites" /></a>

## Psychotherapy

I'm currently training to be a psychotherapy practitioner in [Transactional
Analysis](https://www.goodtherapy.org/learn-about-therapy/types/transactional-analysis) at the [École d'Analyse Transactionnelle de Paris](https://eat-paris.net/).


## Open Source

I'm fond of open source software, and in the past I have been active notably in the [Python](https://www.python.org/) open source community.

Some projects I have contributed to:

* [BeeWare](https://pybee.org)
* [parsel](https://github.com/scrapy/parsel)
* [Scrapy](https://www.scrapy.org)
* [dateparser](https://github.com/scrapinghub/dateparser)
* [cookiecutter-pypackage](https://github.com/audreyr/cookiecutter-pypackage)

For more, check my [GitHub profile](https://github.com/eliasdorneles).

## Writing

Besides writing on [this blog](/), I've also contributed articles to [Python
Help](https://pythonhelp.wordpress.com), a resource in Portuguese for Python
programmers.

I've also translated to Portuguese the book
[Build Your Own Lisp](http://buildyourownlisp.com)
| see
[Construa Seu Próprio Lisp](https://construa-seu-proprio-lisp.herokuapp.com/).
